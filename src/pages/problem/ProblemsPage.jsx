...
const Highlight = ({ text, query }) => {
    ...
    return (
        <>
            {text.slice(0, idx)}
            <mark style={{ background:'rgba(251,191,36,.22)', color:'#fbbf24', borderRadius:2, padding:'0 2px' }}>
...
const ProblemsPage = () => {
    ...
    const { problems, loading, error, fetchAllProblems } = useProblems()
    const { submissions, fetchMySubmissions }            = useSubmission()
    ...
    // ── Build status map from real submissions ────────────────────
    useEffect(() => {
    ...
    const searchId   = isIdSearch ? parseInt(search.replace('#', '')) : null
    const hlQuery    = isIdSearch ? '' : search.trim()
    const diffOrder  = { Easy:0, Medium:1, Hard:2 }

    // ── Get real status for a problem ─────────────────────────────
    const getRealStatus = (p) => problemStatuses[String(p.id)] || p.status || 'none'

    const filtered = sourceData.filter(p => {
        const q = search.toLowerCase().trim()
        const matchSearch = !q ? true
            : isIdSearch ? p.id === searchId
                : p.title.toLowerCase().includes(q) || p.topic.toLowerCase().includes(q)
        const matchDiff   = difficulty  === 'All' || p.difficulty === difficulty
        const matchTopic  = activeTopic === 'All' || p.topic.toLowerCase().includes(activeTopic.toLowerCase())
        const realStatus  = getRealStatus(p)
        const matchStatus = status === 'All'
            || (status === 'Solved'      && realStatus === 'solved')
            || (status === 'Attempted'   && realStatus === 'attempted')
            || (status === 'Not Started' && realStatus === 'none')
    ...
    const getStatusEl = (p) => {
        const s = getRealStatus(p)
        if (s === 'solved')    return <span className="pp-st-success">✓ Attempted Successfully</span>
        if (s === 'attempted') return <span className="pp-st-trying">~ Attempting</span>
        return <span className="pp-st-not-yet">Not Attempted Yet</span>
    }

    const getAccuracy = (p) => {
        const acc = p.accuracy || p.acceptanceRate || 0
        if (!acc || acc === 0) return (
            <span style={{ color:'rgba(148,163,184,.25)', fontFamily:"'JetBrains Mono',monospace", fontSize:12 }}>—</span>
        )
        const pct = Math.round(acc)
        return <span className={accClass(pct)}>{pct}%</span>
    }

    const getRowClass = (p) => {
        const s = getRealStatus(p)
        if (s === 'solved')    return 'pp-row solved-row'
        if (s === 'attempted') return 'pp-row attempted-row'
        return 'pp-row'
    }
    ...
    return (
        ...
        .pp-grid  { position:fixed; inset:0; background:repeating-linear-gradient(90deg,rgba(226,232,240,.018) 0,transparent 1px,transparent 80px),repeating-linear-gradient(180deg,rgba(226,232,240,.018) 0,transparent 1px,transparent 80px); animation:pp-gridFade 5s ease-in-out infinite; pointer-events:none; z-index:0; }
        .pp-glow  { position:fixed; border-radius:50%; filter:blur(100px); pointer-events:none; z-index:0; animation:pp-glowPulse 5s ease-in-out infinite; }
        ...
        .pp-daily-btn   { padding:7px 16px; border-radius:8px; background:rgba(249,115,22,.15); border:1px solid rgba(249,115,22,.25); color:#fb923c; font-size:12.5px; font-weight:700; cursor:pointer; font-family:'Space Grotesk',sans-serif; transition:all .2s; }
        .pp-daily-btn:hover { background:rgba(249,115,22,.25); }

        .pp-search-wrap { margin-bottom:14px; animation:pp-fadeUp .5s ease-out .1s both; }
        .pp-search-bar  { display:flex; align-items:center; gap:10px; padding:11px 16px; border-radius:12px; background:rgba(226,232,240,.04); border:1px solid rgba(226,232,240,.1); transition:all .25s; }
        .pp-search-bar.focused { border-color:rgba(96,165,250,.4); background:rgba(96,165,250,.03); box-shadow:0 0 0 3px rgba(96,165,250,.08); }
        .pp-search-bar input { background:none; border:none; outline:none; color:#e2e8f0; font-size:13.5px; font-family:'JetBrains Mono',monospace; width:100%; }
        .pp-search-bar input::placeholder { color:rgba(148,163,184,.3); }
        .pp-kbd { font-size:10.5px; padding:2px 7px; border-radius:5px; background:rgba(226,232,240,.06); border:1px solid rgba(226,232,240,.1); color:rgba(148,163,184,.4); font-family:'JetBrains Mono',monospace; white-space:nowrap; flex-shrink:0; }
        .pp-clear-btn { display:flex; align-items:center; justify-content:center; width:20px; height:20px; border-radius:50%; background:rgba(226,232,240,.08); border:none; cursor:pointer; color:rgba(148,163,184,.6); font-size:12px; flex-shrink:0; transition:all .2s; }
        .pp-clear-btn:hover { background:rgba(226,232,240,.14); color:#e2e8f0; }
        .pp-search-hints { display:flex; gap:16px; flex-wrap:wrap; padding:8px 4px 4px; }
        .pp-sh  { display:flex; align-items:center; gap:6px; font-size:11.5px; color:rgba(148,163,184,.45); }
        .pp-sh-dot { width:4px; height:4px; border-radius:50%; background:rgba(96,165,250,.4); flex-shrink:0; }
        .pp-sh code { font-family:'JetBrains Mono',monospace; font-size:11px; background:rgba(226,232,240,.06); padding:1px 6px; border-radius:4px; color:rgba(148,163,184,.6); }
        .pp-search-meta { display:flex; align-items:center; justify-content:space-between; margin-bottom:10px; flex-wrap:wrap; gap:8px; }
        .pp-result-count { font-size:12px; color:rgba(148,163,184,.45); font-family:'JetBrains Mono',monospace; }
        .pp-result-count span { color:#60a5fa; font-weight:600; }
        ...
        .pp-thead  { display:grid; grid-template-columns:52px 1fr 120px 110px 85px 180px; padding:10px 16px; background:rgba(226,232,240,.03); border-bottom:1px solid rgba(226,232,240,.07); font-size:10.5px; font-weight:700; color:rgba(148,163,184,.45); letter-spacing:.07em; text-transform:uppercase; font-family:'JetBrains Mono',monospace; }
        .pp-row    { display:grid; grid-template-columns:52px 1fr 120px 110px 85px 180px; padding:13px 16px; border-bottom:1px solid rgba(226,232,240,.05); transition:all .2s; cursor:pointer; align-items:center; animation:pp-rowIn .4s ease-out both; }
        .pp-row:last-child { border-bottom:none; }
        .pp-row:hover { background:rgba(226,232,240,.03); }
        .pp-row.solved-row   { border-left:2px solid rgba(34,197,94,.3); }
        .pp-row.attempted-row{ border-left:2px solid rgba(245,158,11,.25); }
        .pp-row.solved-row:hover   { background:rgba(34,197,94,.02); }
        .pp-row.attempted-row:hover{ background:rgba(245,158,11,.02); }
        ...
        .pp-st-success  { display:flex; align-items:center; gap:5px; color:#22c55e; font-size:11px; font-weight:700; font-family:'JetBrains Mono',monospace; }
        .pp-st-trying   { display:flex; align-items:center; gap:5px; color:#f59e0b; font-size:11px; font-weight:700; font-family:'JetBrains Mono',monospace; }
        .pp-st-not-yet  { color:rgba(148,163,184,.25); font-size:11px; font-family:'JetBrains Mono',monospace; }
        ...
        .pp-empty-btn  { margin-top:14px; padding:7px 18px; border-radius:8px; background:rgba(96,165,250,.1); border:1px solid rgba(96,165,250,.2); color:#60a5fa; font-size:12.5px; cursor:pointer; font-family:'Space Grotesk',sans-serif; }
        .pp-skeleton { height:52px; background:linear-gradient(90deg,rgba(226,232,240,.04) 25%,rgba(226,232,240,.08) 50%,rgba(226,232,240,.04) 75%); background-size:200% auto; animation:pp-skeleton 1.5s linear infinite; border-bottom:1px solid rgba(226,232,240,.05); }
        ...
        @media(max-width:768px){
            .pp-thead,.pp-row { grid-template-columns:40px 1fr 80px 80px; }
            .pp-row>*:nth-child(3),.pp-row>*:nth-child(5),
            .pp-thead>*:nth-child(3),.pp-thead>*:nth-child(5){ display:none; }
        }
            `}</style>
                ...
                <div className="pp-inner">
                    ...
                    <div className="pp-top">
                        <div>
                            ...
                            <div className="pp-h1">Problems <span className="pp-h1-accent">{sourceData.length > 0 ? `${sourceData.length}+` : '250+'}</span></div>
                        ...
                        <div className="pp-solved-wrap">
                            <div className="pp-counts">
                                <span className="pp-sc" style={{background:'rgba(34,197,94,.07)',border:'1px solid rgba(34,197,94,.14)',color:'#22c55e'}}>Easy {easyCount}</span>
                                <span className="pp-sc" style={{background:'rgba(245,158,11,.07)',border:'1px solid rgba(245,158,11,.14)',color:'#f59e0b'}}>Med {medCount}</span>
                                <span className="pp-sc" style={{background:'rgba(239,68,68,.07)',border:'1px solid rgba(239,68,68,.14)',color:'#ef4444'}}>Hard {hardCount}</span>
                            </div>
                    ...
                    <div className="pp-daily" onClick={() => navigate('/problems/daily-challenge')}>
                        <div className="pp-daily-l">
                            ...
                            <div>
                                ...
                                <div className="pp-daily-sub">Longest Palindromic Substring · Medium · +150 pts · ⏱ Ends in 8h 24m</div>
                            </div>
                        </div>
                    ...
                    {error && <div className="pp-error">⚠ {error} — showing mock data</div>}

                    {/* SEARCH BAR */}
                    <div className="pp-search-wrap">
                        <div className={`pp-search-bar ${searchFocus ? 'focused' : ''}`}>
                            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="rgba(148,163,184,.4)" strokeWidth="2.5" style={{flexShrink:0}}>
                            ...
                            />
                            {!search && <span className="pp-kbd">Ctrl K</span>}
                            {search && (
                                <button className="pp-clear-btn" onClick={() => { setSearch(''); setPage(1); searchInputRef.current?.focus() }}>✕</button>
                            )}
                        </div>
                        {!search && (
                            <div className="pp-search-hints">
                                <span className="pp-sh"><span className="pp-sh-dot"/><code>#1</code> or <code>1</code> — search by ID</span>
                                <span className="pp-sh"><span className="pp-sh-dot"/><code>Two Sum</code> — search by title</span>
                                <span className="pp-sh"><span className="pp-sh-dot"/><code>Arrays</code> — search by topic</span>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="pp-filters">
                        <select className="pp-sel" value={difficulty} onChange={e => { setDifficulty(e.target.value); setPage(1) }}>
                            <option value="All">All Difficulty</option>
                            <option value="Easy">Easy</option>
                            <option value="Medium">Medium</option>
                            <option value="Hard">Hard</option>
                        </select>
                        <select className="pp-sel" value={activeTopic} onChange={e => { setActiveTopic(e.target.value); setPage(1) }}>
                            <option value="All">All Topics</option>
                            {TOPICS.slice(1).map(t => <option key={t} value={t}>{t}</option>)}
                        </select>
                        <select className="pp-sel" value={status} onChange={e => { setStatus(e.target.value); setPage(1) }}>
                            <option value="All">All Status</option>
                            <option value="Solved">Attempted Successfully</option>
                            <option value="Attempted">Attempting</option>
                            <option value="Not Started">Not Attempted Yet</option>
                        </select>
                        <select className="pp-sel" value={sortBy} onChange={e => { setSortBy(e.target.value); setPage(1) }}>
                    ...
                    <div className="pp-topics">
                        {TOPICS.map(t => (
                            <div key={t} className={`pp-tc ${activeTopic===t?'on':''}`} onClick={() => { setActiveTopic(t); setPage(1) }}>{t}</div>
                        ))}
                    </div>

                    {/* RESULT COUNT */}
                    <div className="pp-search-meta">
                        <div className="pp-result-count">
                            {search
                                ? <><span>{sorted.length}</span> result{sorted.length!==1?'s':''} for "{search}"</>
                                : <><span>{sorted.length}</span> problem{sorted.length!==1?'s':''}</>
                            }
                        </div>
                    </div>
                    ...
                    <div className="pp-tbl">
                        ...
                        {loading ? (
                            Array(8).fill(0).map((_,i) => (
                                <div key={i} className="pp-skeleton" style={{animationDelay:`${i*.1}s`}}/>
                            ))
                        ) : paginated.length === 0 ? (
                            <div className="pp-empty">
                                <div className="pp-empty-icon">🔍</div>
                                <div className="pp-empty-text">No problems found</div>
                                <div className="pp-empty-sub">{search ? `No results for "${search}"` : 'Try adjusting your filters'}</div>
                                {search && <button className="pp-empty-btn" onClick={() => setSearch('')}>Clear search</button>}
                            </div>
                        ) : (
                            paginated.map((p,i) => (
                                <div
                                ...
                                >
                                    ...
                                    <div className="pp-title-wrap">
                                        <span className="pp-title-text">
                                            <Highlight text={p.title} query={hlQuery}/>
                                        </span>
                                        {p.tag && <span className={`pp-tag-badge ${tagClass(p.tag)}`}>{p.tag}</span>}
                                    </div>
                                    <div className="pp-topic-text">
                                        <Highlight text={p.topic} query={hlQuery}/>
                                    </div>
                                    <div><span className={diffClass(p.difficulty)}>{p.difficulty}</span></div>
                                    <div>{getAccuracy(p)}</div>
                                    <div>{getStatusEl(p)}</div>
                                </div>
                            ))
                        )}
                    </div>
                    ...
                    {!loading && paginated.length > 0 && (
                        <div className="pp-pag">
                            <div className="pp-pag-info">
                                Showing {(page-1)*PER_PAGE+1}–{Math.min(page*PER_PAGE,sorted.length)} of {sorted.length} problems
                            </div>
                            <div className="pp-pag-btns">
                                <div className={`pp-pag-btn ${page===1?'disabled':''}`} onClick={() => setPage(p=>Math.max(1,p-1))}>‹ Prev</div>
                                {Array.from({length:Math.min(totalPages,5)},(_,i) => {
                                    const pn = totalPages<=5?i+1:page<=3?i+1:page>=totalPages-2?totalPages-4+i:page-2+i
                                    return <div key={pn} className={`pp-pag-btn ${page===pn?'on':''}`} onClick={()=>setPage(pn)}>{pn}</div>
                                })}
                                {totalPages>5 && (
                                    <>
                                        <div className="pp-pag-btn">···</div>
                                        <div className={`pp-pag-btn ${page===totalPages?'on':''}`} onClick={()=>setPage(totalPages)}>{totalPages}</div>
                                    </>
                                )}
                                <div className={`pp-pag-btn ${page===totalPages?'disabled':''}`} onClick={() => setPage(p=>Math.min(totalPages,p+1))}>Next ›</div>
                            </div>
                        </div>
                    )}
